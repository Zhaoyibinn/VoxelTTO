...
class CameraDec(nn.Module):
    ...
    def forward(self, feat, camera_encoding=None, *args, **kwargs):
        B, N = feat.shape[:2]
        # Camera decoding is lightweight and numerically sensitive. Keep the
        # complete decoder path in FP32 even when the image backbone runs in
        # BF16/FP16.
        feat = feat.reshape(B * N, -1).float()
        ...
        if camera_encoding is None:
            ...
            out_qvec = camera_encoding[..., 3:7].float()
            out_fov = camera_encoding[..., -2:].float()
