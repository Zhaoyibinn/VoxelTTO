...
class DualDPT(nn.Module):
    ...
    def _forward_impl(
        ...
        # Match VGGT-X's mixed-precision DPT boundary: keep the large feature
        # extraction/fusion path in its original dtype, and run only the final
        # prediction head in FP32.
        main_logits = self.scratch.output_conv2(fused_main.float().contiguous())
        ...
        last_aux_logits = self.scratch.output_conv2_aux[-1](
            last_aux.float().contiguous()
        )
